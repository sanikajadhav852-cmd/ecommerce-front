...
export default function ProductDetail() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center" style={{ backgroundColor: 'var(--background, #ffffff)' }}>
        <Loader size="large" />
        <p className="mt-4 font-medium" style={{ color: 'var(--text-secondary, #6b7280)' }}>
          Loading product...
        </p>
      </div>
    );
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-6" style={{ backgroundColor: 'var(--background, #ffffff)' }}>
        <div className="text-center">
          <h2 className="text-4xl font-bold mb-4" style={{ color: 'var(--text-primary, #111827)' }}>
            {error || "Product Not Found"}
          </h2>
          <Link 
            to="/products" 
            className="font-semibold flex items-center justify-center gap-2 hover:underline"
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: 'var(--background, #ffffff)' }}>
      ...
      <div className="max-w-7xl mx-auto px-6 pt-8">
        <nav className="flex items-center text-sm mb-8" style={{ color: 'var(--text-secondary, #6b7280)' }}>
          <Link to="/" className="hover:underline transition" style={{ color: 'var(--text-primary, #111827)' }}>
            Home
          </Link>
          ...
          <span className="mx-3">/</span>
          <span className="font-medium" style={{ color: 'var(--text-primary, #111827)' }}>
            {product.name}
          </span>
        </nav>
      </div>
      ...
      <main className="max-w-7xl mx-auto px-6 pb-24">
        <div className="grid lg:grid-cols-2 gap-12 xl:gap-16 items-start">
          ...
          <div className="space-y-6">
            <div className="relative aspect-square rounded-3xl overflow-hidden shadow-xl border" style={{
              ...
              <button 
                ...
                className="absolute top-5 right-5 p-4 backdrop-blur-md rounded-full shadow-lg hover:scale-110 transition-all z-10"
            ...
            <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
              {[product.image, ...(product.other_images || [])].map((img, idx) => (
                <button
                  ...
                  style={{ borderColor: activeImage === img ? 'var(--primary, #7c3aed)' : 'transparent' }}
                >
          ...
          <div className="space-y-8 lg:sticky lg:top-8">
            {/* Title & Price */}
            <div>
              <h1 className="text-4xl md:text-5xl font-bold tracking-tight" style={{ color: 'var(--text-primary, #111827)' }}>
                {product.name}
              </h1>
              <div className="mt-4 flex items-baseline gap-4">
                <span className="text-4xl font-bold" style={{ color: 'var(--text-primary, #111827)' }}>
                  ₹{currentPrice.toLocaleString('en-IN')}
                </span>
                {discountPercentage > 0 && (
                  <span className="text-xl line-through" style={{ color: 'var(--text-secondary, #6b7280)' }}>
            ...
            {availableWeights.length > 0 && (
              <div className="space-y-4">
                <label className="block text-sm font-semibold uppercase tracking-wide" style={{ color: 'var(--text-secondary, #6b7280)' }}>
                  Select Pack Size
                </label>
                  ...
                  {availableWeights.map(weight => {
                    ...
                    return (
                      <button
                        ...
                        `}
                        style={{
                          backgroundColor: isSelected ? 'var(--primary, #7c3aed)' : 'var(--surface, #ffffff)',
                          color: isSelected ? 'white' : 'var(--text-primary, #111827)',
                          borderColor: isSelected ? 'var(--primary, #7c3aed)' : 'var(--gray-light, #e5e7eb)',
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center border rounded-full w-fit overflow-hidden shadow-sm" style={{ borderColor: 'var(--gray-light, #e5e7eb)' }}>
                <button
                  onClick={decreaseQty}
                  disabled={quantity <= 1}
                  className="px-5 py-3 transition disabled:opacity-40"
                  style={{ color: 'var(--text-secondary, #6b7280)' }}
                >
                  <Minus size={18} />
                </button>
                <span className="px-6 py-3 text-lg font-medium border-x" style={{ borderColor: 'var(--gray-light, #e5e7eb)' }}>
                  {quantity}
                </span>
                <button
                  onClick={increaseQty}
            ...
            <div className="pt-6">
              <button
                ...
                style={{
                  backgroundColor: selectedVariant?.stock > 0 ? 'var(--primary, #7c3aed)' : 'var(--gray-medium, #9ca3af)',
                  color: 'white',
                }}
              >
                {selectedVariant?.stock > 0 ? (
            ...
            {/* Description */}
            {product.description && (
              <div className="pt-8 border-t" style={{ borderColor: 'var(--gray-light, #e5e7eb)' }}>
                <h3 className="text-lg font-semibold mb-4" style={{ color: 'var(--text-primary, #111827)' }}>
                  Description
                </h3>
                <p className="leading-relaxed" style={{ color: 'var(--text-secondary, #6b7280)' }}>
                  {product.description}
                </p>
              </div>
            )}
